...
class TaskSensor(SensorEntity, RestoreEntity):
    ...
    def _check_and_fire_due_event(self, old_state):
        ...
        # Only "Overdue" is actually missed - "Due Today" is just a
        # heads-up that today is the day, not an alert. Fire on any real
        # transition into "Overdue", regardless of what the state was
        # before (Due Today, Due in N days, Snoozed, ...), but not on a
        # no-op recompute while already Overdue.
